	...
	{:else if displayData}
		<div class="space-y-6">
			...
			<div>
				...
				<div class="mt-1 font-mono text-lg font-bold break-all text-white">
			...
			{#if bundle}
				<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
					<!-- Make -->
					<div>
						<div class="text-xs font-bold tracking-wider text-slate-500 uppercase">Make</div>
						<div class="mt-1 text-base font-medium text-white">{bundle.make || 'Unknown'}</div>
					</div>

					<!-- Model -->
					<div>
						<div class="text-xs font-bold tracking-wider text-slate-500 uppercase">Model</div>
						<div class="mt-1 text-base font-medium text-white">{bundle.model || 'Unknown'}</div>
					</div>

					<!-- Years -->
					<div>
						<div class="text-xs font-bold tracking-wider text-slate-500 uppercase">Years</div>
				...
				<div class="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
					...
					<div class="max-h-60 space-y-1 overflow-y-auto pr-2">
						{#each matchingCars as car}
							<div class="border-b border-slate-700/50 py-1 text-xs text-slate-300 last:border-0">
								{car.name}
							</div>
						{/each}
					</div>
			...
			<div class="border-t border-slate-700/50 pt-4">
				<details class="group">
					<summary class="cursor-pointer font-mono text-xs text-slate-500 hover:text-slate-300"
		...
		<div class="flex flex-1 flex-col items-center justify-center space-y-4 py-10 text-center">
			...
			<div>
				...
				<p class="mt-1 text-sm text-slate-400">
					Fingerprint: <code class="rounded bg-slate-800 px-1 py-0.5">{fingerprint}</code>
				</p>
				<p class="mt-2 text-xs text-slate-500">This platform is not in the known vehicle list.</p>
			</div>
		</div>
	{:else}
