	...
	<div class="flex flex-1 flex-col gap-4 p-4 sm:flex-row sm:items-center sm:py-6 sm:pr-6 sm:pl-8">
		...
		<div class="mt-2 flex shrink-0 sm:mt-0">
			{#if !isLoading}
				{#if mode === 'manual'}
					<button
						class="flex cursor-pointer items-center gap-2 rounded-lg bg-red-500/10 px-4 py-2 text-sm font-bold text-red-400 transition-colors hover:bg-red-500/20 hover:text-red-300"
						onclick={(e) => {
							e.stopPropagation();
							onRemove();
						}}
