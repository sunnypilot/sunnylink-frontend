<script lang="ts">
	...
	let hasContent = $derived(
		useSections ? orderedSections.length > 0 : itemGroups.length > 0 || allSubPanels.length > 0
	);
		...
		{#if useSections}
			...
			{#each orderedSections as section, si (section.id)}
				...
				{@const sectionHasContent =
					visibleItems.length > 0 || getAllSectionSubPanels(section).length > 0}
				...
				{#if sectionHasContent}
					...
					<div class:mt-8={si > 0}>
						...
						{#if visibleItems.length > 0 || getAllSectionSubPanels(section).length > 0}
							...
							{@const orphans = getAllSectionSubPanels(section).filter(
								(sp) => !visibleItems.some((i) => i.key === sp.trigger_key)
							)}
								...
								{#each visibleItems as item, i (item.key)}
									...
									{#if spMap[item.key] !== undefined}
										...
										<button
											...
											disabled={!spEnabled}
											onclick={() => {
												const s = spMap[item.key];
												if (s) onSubPanelOpen?.(s);
											}}
										>
											<span class="text-[0.8125rem] font-medium text-[var(--sl-text-1)]"
												>{spMap[item.key]?.label}</span
											>
						...
						{#each group.items as item, i (item.key)}
							...
							{#if subPanelByTrigger[item.key] !== undefined}
								...
								<button
									...
									disabled={!spEnabled}
									onclick={() => {
										const s = subPanelByTrigger[item.key];
										if (s) onSubPanelOpen?.(s);
									}}
								>
									<span class="text-[0.8125rem] font-medium text-[var(--sl-text-1)]"
										>{subPanelByTrigger[item.key]?.label}</span
									>
									<ChevronRight size={16} class="text-[var(--sl-text-3)]" />
								</button>
